...
@property (nonatomic, strong)MKPolyline *nowPolyline;//当前已经添加的覆盖层模型
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations{
    ...
    //初始化覆盖层模型
    MKPolyline *line = [MKPolyline polylineWithCoordinates:coordinates count:self.lines.count];
    //地图添加
...
-(MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation{
    ...
    if ([annotation isKindOfClass:[QYAnnotation class]]) {
        ...
        //指定一个标识符，从复用队列出对
        MKAnnotationView *view =[mapView dequeueReusableAnnotationViewWithIdentifier:idifiter];
        if (!view) {
            view = [[MKAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:idifiter];
        }
